function [curve, tang, norm, bino]=curve_vdefortang_anycurvature_function(s,s0,kap)
...
% Tangent vector
tang(1,:)=F_T0-((kap^2/2)*F_T2);
tang(2,:)=kap*F_T1;
tang(3,:)=((kap^2)/(2*s0))*F_T2;

% Principal normal vector
norm(1,:)=((1/kap)*F_N0)-((kap/2)*F_N2);
norm(2,:)=F_N1;
norm(3,:)=(kap/(2*s0))*F_N2;
...
% Curve
curve(1,:)=F_curve0-(kap^2/2)*F_curve2;
curve(2,:)=kap*F_curve1;
curve(3,:)=((kap^2)/(2*s0))*F_curve2;
end